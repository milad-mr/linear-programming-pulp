...
X_train, _, y_train, _ = train_test_split(data[:,:-1], data[:,-1], test_size=0.4, random_state=21)
# print("xtrain old is:", y_train)

_, X_test, _, y_test = train_test_split(data[:,:-1], data[:,-1], test_size=.99, random_state=21, shuffle=False)
...
print("Y is", y)
model += pulp.lpSum([1*v1[i] + 1*v2[i] + ep1[i] + ep2[i] for i in range(len(x))]), "Optimization Function"
...
print("Status:", LpStatus[model.status], "lambda:", lamda, "rate.4", "time:", model.solutionTime)
plt.show()